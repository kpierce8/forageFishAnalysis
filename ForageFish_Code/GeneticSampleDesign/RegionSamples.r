shell(paste("mkdir C:\\data\\rplots\\plottemp", gsub('-','',Sys.Date()),"\\RegionSamples", sep=""))
script <- "\\RegionSamples"
cscheme <- 0
par(par_reset)
...
colram<-sort(unique(sm_surveys$Sampleregion))
par(ps=20)
boxplot(ffsall$CosMos ~ as.factor(ffsall$Sampleregion),col=reramp[colram],main="Surveys by Region", xaxt="n")
...
boxplot(sm_surveys$CosMos ~ as.factor(sm_surveys$Sampleregion),col=reramp[colram],main="Surf Smelt by Region", xaxt="n")
axis(1,at=c(1:7),seq(1,7))
savePlot(paste(pldir,script,"\\SmeltByRegion.wmf",sep=""),type="wmf")
boxplot(la_surveys$CosMos ~ as.factor(la_surveys$Sampleregion),col=reramp[colram],main="Sand Lance by Region",ylim=c(-1,1),xaxt="n")
axis(1,at=c(1:7),seq(1,7))
savePlot(paste(pldir,script,"\\LanceByRegion.wmf",sep=""),type="wmf")


#boxplot(points$CosMos ~ as.factor(points$SampleZone),col=c(3,2))
#table(points$SampleZone)
#subhex <- subset(points,points$SampleZone == 2)
#boxplot(subhex$CosMos)
#subhex$CosMos
#subhex$Month_
#table(subhex$Month_)
#subhex <- subset(points,points$SampleZone == 4)
#table(subhex$Month_)
#subhex <- subset(ffsall,ffsall$Sampleregion == 4)
#table(subhex$Month_)
#
##both_surveys <- subset(la_surveys,la_surveys$Smelt_Ind ==1)
##boxplot(both_surveys$CosMos ~ as.factor(both_surveys$Sampleregion),col=reramp[colram],main="Both by Region",ylim=c(-1,1))
##savePlot(paste(pldir,script,"\\LanceByRegion.wmf",sep=""),type="wmf")
##
both_surveys <- subset(la_surveys,la_surveys$Smelt_Ind ==1)
dim(both_surveys)
