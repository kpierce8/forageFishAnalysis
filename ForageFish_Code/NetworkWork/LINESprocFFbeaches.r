library(igraph)
library(foreign)
source( "e:/foragefish/HydroNetworks/Trunk/NetFunctions.r")

wria <- c("SZ")
LINEID <- c("UNIT_ID") #wchydro is LLID
LENGTH <- c("Shape_Length") #wchydro is LLID
library(RODBC)
cnn1 <- odbcConnectAccess("e:/ForageFish/Pierce/myForageFish.mdb")
...
dim(linesnet.filt)
table.norow(linesnet.filt[,3:4],paste("f:/foragefish/wchydroGR",wria,".txt",sep=""),col.names=F)
##################################################################################


##Import Graph and add identifier attributes
graphset<- read.graph(paste("f:/foragefish/wchydroGR",wria,".txt",sep=""))
...
write.dbf(lines.dbf.clust,paste("C:/data/temp/edgetest2",wria,".dbf",sep=""))
write.dbf(temp3,paste("C:/data/temp/nodestest",wria,".dbf",sep=""))
